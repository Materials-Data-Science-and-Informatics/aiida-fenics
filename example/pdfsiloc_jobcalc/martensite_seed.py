...
from pfdisloc.phase_field_models import (
    LevitasModel, SchmittModel
)

from pfdisloc.solvers import (
    LinearElasticitySolver, HigherOrderElasticitySolver
)
...
try:
    ...
    raise SystemExit('ERROR: Invalid configuration in \'{}\':\n{}'
                     .format(args.config_file.name, err))

mesh = fenics.RectangleMesh(fenics.Point(config.domain.size) * -0.5,
                            fenics.Point(config.domain.size) * 0.5,
                            config.domain.mesh_size[0],
                            config.domain.mesh_size[1],
                            'crossed')

if config.domain.refinement:
    refine_range = 0.5 * min(config.domain.size[0] /
                             config.domain.mesh_size[0],
...
model = BinaryPhaseModel(base_model,
                         config.materials,
...
if config.phase_field.parameters.model == 'schmitt':
    ...
    raise NotImplementedError('phase field model \'{}\' not yet supported'
                              .format(config.phase_field.parameters.model))

pf_solver = PhaseFieldSolver(V, order_parameter=martensite_order_parameter,
                             model=pf_model)
...
order_parameter_output = fenics.XDMFFile(f'{output_base_path}/order_parameter.xdmf')
...
elastic_energy_density_output = fenics.XDMFFile(f'{output_base_path}/elastic_energy_density.xdmf')
...
if isinstance(base_model, DislocationModel):
    dislocation_trace_output = fenics.XDMFFile(f'{output_base_path}/dislocation_trace.xdmf')
...
if true_stress:
    ...
    ho_solver.solve_phasefield(linear_stress,
                               config.materials,
                               pf_solver.order_parameter(),
                               pf_model)
...
elastic_energy_density.assign(fenics.project(
    0.5 * fenics.inner(elastic_solver.stress(),
                       elastic_solver.strain()),
